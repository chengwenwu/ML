...
float  Matrix::algebraicComplment(int row, int col)
{
	...
	float *newarray = new float[(int)pow(this->cols - 1, 2)];
	int count = 0;
...
float  Matrix::detSub(Matrix &matrix)
{
	...
	{
		float *newarray = new float[(int)pow(matrix.cols - 1 , 2)];
		int count = 0;
...
*/
 ...
 {
	 if (!(vec1.getRows() == vec2.getRows() && vec1.getCols() == 1 && vec2.getCols() == 1))
	 {
		 cout << "vec1 and vec2 should be vector. from  float eDist(const Matrix vec1, const Matrix vec2)\n";
		 system("pause");
	 }
	 Matrix tempVec = vec1 - vec2;
	 Matrix temp2 = tempVec.trans()*tempVec ;
	 return sqrt(temp2.matrixToFloat());
 }

 /*
*重载-号
*/
 Matrix operator-(const Matrix& matrix_in1, const Matrix& matrix_in2)
 {
	 if ( matrix_in1.rows != matrix_in2.rows ||  matrix_in1.cols !=  matrix_in2.cols)
	 {
		 cout << "Dimension error from Matrix  Matrix::operator+(const Matrix&  matrix_in2)\n";
		 system("pause");
	 }
	 Matrix newMatrix( matrix_in1.rows,  matrix_in1.cols);
	 for (int counter0 = 0; counter0 < newMatrix.rows; counter0++)
	 {
		 for (int counter1 = 0; counter1 < newMatrix.cols; counter1++)
		 {
			 newMatrix.elements[counter0][counter1] =  matrix_in1.elements[counter0][counter1] -  matrix_in2.elements[counter0][counter1];
		 }
	 }
	 return newMatrix;
 }
